...
─────────────────────                    ─────────────────────────────────
.pi/extensions/                    部署    ~/.pi/agent/extensions/
...
2. **全局级**：`<agentDir>/extensions/`，即 `~/.pi/agent/extensions/`
...
jishu hub 负责把本仓库的 `.pi/extensions/request-user-input.ts` 部署到目标机器的全局扩展目录 `~/.pi/agent/extensions/request-user-input.ts`。这样无论在哪个项目目录运行 pi，该工具都会被自动加载。
